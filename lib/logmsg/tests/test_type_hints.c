...
Test(type_hints, test_int32_nonzero_terminated)
{
  ...
  cr_assert(type_cast_to_int32("12345", 3, &int32_value, &error), "Type cast of non-zero terminated \"123\" to gint32 failed");
  cr_assert_eq(int32_value, 123);
  cr_assert_null(error);

  cr_assert(type_cast_to_int64("12345", 3, &int64_value, &error), "Type cast of non-zero terminated \"123\" to gint64 failed");
  cr_assert_eq(int64_value, 123);
  cr_assert_null(error);

  cr_assert(type_cast_to_boolean("12", 1, &bool_value, &error), "Type cast of non-zero terminated \"1\" to gboolean failed");
  cr_assert_eq(bool_value, 1);
  cr_assert_null(error);

  cr_assert(type_cast_to_double("123456", 3, &dbl_value, &error), "Type cast of non-zero terminated \"123\" to gdouble failed");
  cr_assert(dbl_value - 123 < 0.1);
  cr_assert_null(error);

  cr_assert(type_cast_to_datetime_unixtime("1699134067.123", 12, &ut_value, &error), "Type cast of non-zero terminated \"1699134067.1\" to unixtime failed");
